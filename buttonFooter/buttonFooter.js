...
export default class ButtonFooter extends NavigationMixin(LightningElement) {
    ...
    calculateGrandTotals() {
        ...
        for (let item of this.lineItemTotals.values()) {
            // FIX: Har value ko jodte waqt force karke number (float) mein convert karein
            totalSumPrice += parseFloat(item.price || 0);
            console.log("Total Sum Price is", totalSumPrice)
            totalSumCost += parseFloat(item.cost || 0);   
            console.log("Total Sum Cost is", totalSumCost)
        }
        ...
        console.log("Total grand total price is", this.grand_total_price)
        console.log("Total grand total cost is", this.grand_total_cost)
    }
